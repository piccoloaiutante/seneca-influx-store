      ...
      if(dbInst){
        ...
        if(!_.isEmpty(args.q)){
          ...
          for (prop in qq) {
            ...
            whereConditions = whereConditions + " " + prop + " = " + val + " AND ";
          }
        ...
        retFields = mq.fields ? mq.fields.join(',') : "*";

        var whereClause = whereConditions != "" ? " WHERE " + whereConditions : "";
       
        var query = "SELECT " + retFields + " FROM " + seriesName + whereClause;
        ...
        dbInst.query(query, function(err, entities){
          ...
          if(err){
            ...
            if( mq.sort ) {
              for( var sf in mq.sort ) break;
              var sd = mq.sort[sf] < 0 ? -1 : 1
            ...
            if( mq.skip ) {
              list = list.slice(mq.skip)
            }

            if( mq.limit ) {
              list = list.slice(0, mq.limit)
            }
            return cb(null, list);
          }
        });
